/****************************************************************************
** Meta object code from reading C++ file 'AkariModel.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.15.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include <memory>
#include "../../jeu_akiri_qt/include/AkariModel.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'AkariModel.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.15.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_AkariModel_t {
    QByteArrayData data[14];
    char stringdata0[142];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_AkariModel_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_AkariModel_t qt_meta_stringdata_AkariModel = {
    {
QT_MOC_LITERAL(0, 0, 10), // "AkariModel"
QT_MOC_LITERAL(1, 11, 20), // "responseOnMouseClick"
QT_MOC_LITERAL(2, 32, 0), // ""
QT_MOC_LITERAL(3, 33, 18), // "Matrix<cellState>&"
QT_MOC_LITERAL(4, 52, 6), // "isDone"
QT_MOC_LITERAL(5, 59, 8), // "set_size"
QT_MOC_LITERAL(6, 68, 10), // "size_index"
QT_MOC_LITERAL(7, 79, 9), // "set_level"
QT_MOC_LITERAL(8, 89, 11), // "level_index"
QT_MOC_LITERAL(9, 101, 13), // "onCellClicked"
QT_MOC_LITERAL(10, 115, 3), // "row"
QT_MOC_LITERAL(11, 119, 3), // "col"
QT_MOC_LITERAL(12, 123, 13), // "ONDoneClicked"
QT_MOC_LITERAL(13, 137, 4) // "unDo"

    },
    "AkariModel\0responseOnMouseClick\0\0"
    "Matrix<cellState>&\0isDone\0set_size\0"
    "size_index\0set_level\0level_index\0"
    "onCellClicked\0row\0col\0ONDoneClicked\0"
    "unDo"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_AkariModel[] = {

 // content:
       8,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   49,    2, 0x06 /* Public */,
       4,    1,   52,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       5,    1,   55,    2, 0x0a /* Public */,
       7,    1,   58,    2, 0x0a /* Public */,
       9,    2,   61,    2, 0x0a /* Public */,
      12,    0,   66,    2, 0x0a /* Public */,
      13,    0,   67,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    2,
    QMetaType::Void, QMetaType::Bool,    2,

 // slots: parameters
    QMetaType::Void, QMetaType::Int,    6,
    QMetaType::Void, QMetaType::Int,    8,
    QMetaType::Void, QMetaType::Int, QMetaType::Int,   10,   11,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void AkariModel::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        auto *_t = static_cast<AkariModel *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->responseOnMouseClick((*reinterpret_cast< Matrix<cellState>(*)>(_a[1]))); break;
        case 1: _t->isDone((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 2: _t->set_size((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: _t->set_level((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->onCellClicked((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 5: _t->ONDoneClicked(); break;
        case 6: _t->unDo(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            using _t = void (AkariModel::*)(Matrix<cellState> & );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&AkariModel::responseOnMouseClick)) {
                *result = 0;
                return;
            }
        }
        {
            using _t = void (AkariModel::*)(bool );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&AkariModel::isDone)) {
                *result = 1;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject AkariModel::staticMetaObject = { {
    QMetaObject::SuperData::link<QObject::staticMetaObject>(),
    qt_meta_stringdata_AkariModel.data,
    qt_meta_data_AkariModel,
    qt_static_metacall,
    nullptr,
    nullptr
} };


const QMetaObject *AkariModel::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *AkariModel::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_AkariModel.stringdata0))
        return static_cast<void*>(this);
    return QObject::qt_metacast(_clname);
}

int AkariModel::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 7)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 7;
    }
    return _id;
}

// SIGNAL 0
void AkariModel::responseOnMouseClick(Matrix<cellState> & _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void AkariModel::isDone(bool _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(std::addressof(_t1))) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
